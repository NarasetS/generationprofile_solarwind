...
app.layout = html.Div([
    ...
    dcc.Dropdown([i for i in range(2002,2021,1)], 2019, id='year-dropdown'),

    html.Header('Grid size (x*y) = 0.25*0.25'),

    html.Header('Country >> Thailand'),

    html.Header('Select UTC Adjustment = +7'),
...
def trigger_extract_data(n_clicks,geojsondata,geojsondata2,planttype,year):
   ...
   ####### Merge geojson from several sources to create geodataframe ########
    ...
    for i in createdatelist(year) :
        path = 'CDS_Data_validation\\'+ str(i) + ".nc"
        print(path)
